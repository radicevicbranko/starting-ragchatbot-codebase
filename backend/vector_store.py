...
class VectorStore:
    ...
    def _resolve_course_name(self, course_name: str) -> str | None:
        """Use vector search to find best matching course by name with similarity threshold"""
        try:
            ...
            if (
                ...
                # Check similarity threshold (ChromaDB uses distance, lower is better)
                # Distance of 1.0+ indicates poor match for short text like course names
                distance = results["distances"][0][0]

                # For course names, be more strict with matching
                SIMILARITY_THRESHOLD = 0.8  # Allow moderate similarity
                ...
                if distance <= SIMILARITY_THRESHOLD:
                    ...
                    print(
                        f"Course name '{course_name}' rejected due to poor similarity (distance: {distance})"
                    )
    ...
    def _build_filter(
        self, course_title: str | None, lesson_number: int | None
    ) -> dict | None:
    ...
    def get_course_outline(self, course_title: str) -> dict[str, Any] | None:
        """
        Get complete course outline including title, link, and all lessons.

        Args:
            course_title: The course title to get outline for
        ...
        try:
            # First resolve the course name to handle fuzzy matching
            resolved_title = self._resolve_course_name(course_title)
            if not resolved_title:
                return None

            # Get course by resolved title (title is the ID)
            ...
            if results and "metadatas" in results and results["metadatas"]:
                ...
                return {
                    ...
                    "lessons": lessons,  # Already parsed list of lesson objects
                }
            return None
        except Exception as e:
